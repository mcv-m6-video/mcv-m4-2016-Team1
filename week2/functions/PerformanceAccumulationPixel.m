function [pixelTP, pixelFP, pixelFN, pixelTN] = PerformanceAccumulationPixel(pixelCandidates, pixelAnnotation)
    ...
    % Binarize the detections
    pixelCandidates = pixelCandidates>0;
    %pixelAnnotation = pixelAnnotation>0;
    
    % Compute TP, FP, FN and TN
    pixelTP = sum(sum(pixelCandidates>0 & pixelAnnotation==255));
    pixelFP = sum(sum(pixelCandidates>0 & pixelAnnotation<=50));
    pixelFN = sum(sum(pixelCandidates==0 & pixelAnnotation==255));
    pixelTN = sum(sum(pixelCandidates==0 & pixelAnnotation<=50));
end
