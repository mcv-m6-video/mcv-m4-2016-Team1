...
%     forEstim_highway_no_shadows = remove_shadows(forEstim_highway,seq_input_highway(t1_h:end),mean_h,color_space);
%     forEstim_fall_no_shadows = remove_shadows(forEstim_fall,seq_input_fall(t1_f:end),mean_f,color_space);
    forEstim_traffic_no_shadows = remove_shadows(forEstim_traffic,seq_input_traffic(t1_t:end),mean_t,color_space);

    % Evaluation functions for TASK 1 (TASK 2 and TASK 3)
%     [TP_h(i,j), FP_h(i,j), FN_h(i,j), TN_h(i,j), Precision_h(i,j), Accuracy_h(i,j), Specificity_h(i,j), Recall_h(i,j), F1_h(i,j)] = task2(forEstim_highway_no_shadows,gt_input_highway(t1_h:end));
%     [TP_f(i,j), FP_f(i,j), FN_f(i,j), TN_f(i,j), Precision_f(i,j), Accuracy_f(i,j), Specificity_f(i,j), Recall_f(i,j), F1_f(i,j)] = task2(forEstim_fall_no_shadows,gt_input_fall(t1_f:end));
    [TP_t(i,j), FP_t(i,j), FN_t(i,j), TN_t(i,j), Precision_t(i,j), Accuracy_t(i,j), Specificity_t(i,j), Recall_t(i,j), F1_t(i,j)] = task2(forEstim_traffic_no_shadows,gt_input_traffic(t1_t:end));
    end
end
