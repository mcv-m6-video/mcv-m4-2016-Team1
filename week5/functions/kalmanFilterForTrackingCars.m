...
function kalmanFilterForTrackingCars
...
% green car moving from left to right on the floor.
showDetections();

%%
% The white region over the car highlights the pixels detected using
% |vision.ForegroundDetector|, which separates moving objects from the
% background. The background subtraction only finds a portion of the car
% because of the low contrast between the car and the floor. In other
...
%%
% # The region's center is usually different from the car's center. In
%   other words, there is an error in the measurement of the car's
%   location.
% # The location of the car is not available when it is occluded by the
...
% * Use |predict| method by itself to estimate car's location when
...
%%
% * When the car is detected, the Kalman filter first predicts its state
...
% * When the car is missing, the Kalman filter solely relies on its
%   previous state to predict the car's current location.

%%
% You can see the car's trajectory by overlaying all video frames.
...
%%
% Notice that the car emerged in a spot that is quite different from the
% predicted location. From the time when the car was released, it was
...
%%
% Detect and annotate the car in the video.
...
%%
% Detect the car in the current video frame.
...
%%
% Show trajectory of the car by overlaying all video frames on top of 
% each other.
...
% show the trajectory of the car.
